...
class Equation {
	...
	Equation(String s) {
		...
		for(String r in reactantStr.split(' + ')) {
			...
			if(Element.exists(r.substring(i, j))) reactants.add(MapEntry(Element.from(r.substring(i, j)),  1));
			else reactants.add(MapEntry(Compound(r.substring(i)), number));
		}
		if(productStr != null) {
			for(String p in productStr.split(' + ')) {
				...
				if(Element.exists(p.substring(i, j))) products.add(MapEntry(Element.from(p.substring(i, j)), 1));
		...
		switch(type) {
          ...
          this.reactants[i] = MapEntry(this.reactants[i].key, counts[i].toInt());
			...
			case Type.compAcid: // No balancing required
				break;
			case Type.compBase:
				this.reactants[0] = MapEntry(reactants[0].key, reactants[1].key.compoundUnits[1].value);
				this.products[0] = MapEntry(this.products[0].key, reactants[1].key.compoundUnits[0].value);
				break;
			...
			case Type.decomp:
				...
				bool halfElement = false;
				for(int i = 0; i < counts.length - 1; i++) {
					counts[i] = this.reactants[0].key.compoundUnits[i].value / this.products[i].value;
					if(counts[i] != counts[i].toInt()) halfElement = true;
				}
				if(halfElement) counts = counts.map((count) => count *= 2).toList();
				for(int i = 0; i < this.products.length; i++) this.products[i]=  MapEntry(this.products[i].key, counts[i].toInt());
				this.reactants[0] = MapEntry(this.reactants[0].key, counts[counts.length - 1].toInt());
				break;
			case Type.decompAcid: // No balancing required
				break;
			case Type.decompBase:
				this.reactants[0] = MapEntry(this.reactants[0].key, products[1].key.compoundUnits[0].value);
				this.products[0] = MapEntry(this.products[0].key, reactants[0].value * reactants[0].key.compoundUnits[1].value ~/ 2);
				break;
			...
			case Type.combustion:
				...
				counts[1] = (products[0].key.compoundUnits[1].value * counts[2] +
										products[1].key.compoundUnits[1].value * counts[3]).toDouble();
	...
	static List<MapEntry> getProducts(List<MapEntry> reactants, Type type) {
		switch(type) {
			case Type.comp:
				...
				int count1;
				if(ionic) {
					int lcmCharge = lcm(reactants[0].key.charge, reactants[1].key.charge).abs();
					count0 = lcmCharge ~/ ((reactants[0].key.charge == 0) ? 1 : reactants[0].key.charge);
					count1 = -lcmCharge ~/ ((reactants[1].key.charge == 0) ? 1 : reactants[1].key.charge);
				} else {
					count0 = reactants[0].key.count;
					count1 = reactants[0].key.count;
				}
				return [MapEntry(Compound.fromUnits([
		  ...
		  case Type.compAcid:
				return [MapEntry(Compound.fromUnits([
					...
					MapEntry(Element.from('O'), reactants[1].key.compoundUnits[1].value + 1),
        ]), 1)];
		    break;
		  case Type.compBase:
				return [MapEntry(Compound.fromUnits([
					MapEntry(reactants[1].key.compoundUnits[0].key, 1),
					MapEntry(Compound('OH'), reactants[1].key.compoundUnits[0].key.charge),
        ]), 1)];
		    break;
		  case Type.compSalt:
				return [MapEntry(Compound.fromUnits([
					MapEntry(reactants[0].key.compoundUnits[0].key, reactants[0].key.compoundUnits[0].value),
					MapEntry(reactants[1].key.compoundUnits[0].key, reactants[1].key.compoundUnits[0].value),
					MapEntry(Element.from('O'), reactants[0].key.compoundUnits[1].value + reactants[1].key.compoundUnits[1].value),
				]), 1)];
		    break;
		  case Type.decomp:
				return [
          MapEntry(Element.from(reactants[0].key.compoundUnits[0].key.formula), 1),
          MapEntry(Element.from(reactants[0].key.compoundUnits[1].key.formula), 1)];
		    break;
		  ...
		  case Type.decompSalt:
		  	Compound metalOxide = Compound.fromUnits([
					MapEntry(reactants[0].key.compoundUnits[0].key, reactants[0].key.compoundUnits[0].value),
					MapEntry(Element.from('O'), (reactants[0].key.compoundUnits[0].value * reactants[0].key.compoundUnits[0].key.charge) ~/ 2),
        ]);
				Compound nonmetalOxide = Compound.fromUnits([
					MapEntry(reactants[0].key.compoundUnits[1].key, reactants[0].key.compoundUnits[1].value),
					MapEntry(Element.from('O'), reactants[0].key.compoundUnits[2].value - metalOxide.compoundUnits[1].value),
        ]);
				return [MapEntry(metalOxide, 1), MapEntry(nonmetalOxide, 1)];
		    break;
		  case Type.combustion:
		    return [MapEntry(Compound('H2O(g)'), 1), MapEntry(Compound('CO2(g)'), 1)];
		    break;
		  case Type.singleReplacement:
		    if(reactants[0].key.metal) return [
		    	MapEntry(Compound.fromUnits([
						MapEntry(reactants[0].key, reactants[1].key.compoundUnits[0].value),
						MapEntry(reactants[1].key.compoundUnits[1].key, reactants[1].key.compoundUnits[1].value)
          ]), 1),
          MapEntry(reactants[1].key.compoundUnits[0].key, 1),
        ];
		    else return [
			...
			if(reactants[0].key.isElement()) return null;
				if(reactants[0].key.compoundUnits[0].key.equals('H')) {
					if(reactants[0].key.compoundUnits[2].key.equals('O')) {
						if(!reactants[0].key.compoundUnits[1].key.metal) return Type.decompAcid;
					}
				}
				else if(reactants[0].key.compoundUnits[0].key.metal) {
					if(reactants[0].key.compoundUnits[1].key.isCompound()) {
						if(reactants[0].key.compoundUnits[1].key.formula.compareTo('OH') == 0) return Type.decompBase;
					}
      ...
      if(reactants[0].key.compoundUnits[0].key.isElement()) {
			...
			if(reactants[0].key.formula.compareTo('H2O(l)') == 0) {
				if(reactants[1].key.compoundUnits[1].key.equals('O')) {
					if(!reactants[1].key.compoundUnits[0].key.metal) return Type.compAcid;
					return Type.compBase;
				}
			}
			else if(reactants[0].key.compoundUnits[1].key.equals('O') && reactants[1].key.compoundUnits[1].key.equals('O')) {
